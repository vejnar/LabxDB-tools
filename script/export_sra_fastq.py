...
#
# Copyright (C) 2016-2020 Charles E. Vejnar
#
...
def rezip(path_in, fname_out):
    ...
    time.sleep(10)
    subprocess.run(['gzip', '-9', fname_out], check=True)
...
def main(argv=None):
    ...
    for record in data:
        if record['type'] == 'single':
            gin = glob.glob(os.path.join(config['path_seq_run'], record['run_ref'], '*%s.zst'%record['pattern']))
            if len(gin) == 1:
                path_in = gin[0]
            else:
                raise ValueError('More than one file found')
            fname_out = '%s%s'%(record['run_ref'], record['pattern'])
            jobs.append([path_in, fname_out])
        elif record['type'] == 'multiplex':
            gin = glob.glob(os.path.join(config['path_seq_raw'], record['flowcell'], '%s*%s.zst'%(record['tube_label'], record['pattern'])))
            if len(gin) == 1:
                path_in = gin[0]
            else:
                raise ValueError('More than one file found')
            fname_out = '%s_%s%s'%(record['flowcell'], record['tube_label'], record['pattern'])
            jobs.append([path_in, fname_out])
